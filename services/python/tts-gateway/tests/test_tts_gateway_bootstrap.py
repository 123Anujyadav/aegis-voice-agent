"""Bootstrap tests for the tts-gateway service.
...
import pytest
from tts_gateway.config import TtsGatewayConfig
...
def test_config_loads_with_defaults() -> None:
    ...
    config = load_config(TtsGatewayConfig, service_name="tts-gateway")

    assert config.service_name == "tts-gateway"
...
def test_reads_its_own_env_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CS_TTS_GATEWAY_SERVICE_NAME", "tts-gateway")
    monkeypatch.setenv("CS_TTS_GATEWAY_HTTP_PORT", "9100")

    config = load_config(TtsGatewayConfig)
...
def test_rejects_colliding_ports() -> None:
    ...
    with pytest.raises(ConfigurationError):
        load_config(TtsGatewayConfig, service_name="tts-gateway", http_port=8080, health_port=8080)
