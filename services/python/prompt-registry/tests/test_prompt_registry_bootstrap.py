"""Bootstrap tests for the prompt-registry service.
...
def test_config_loads_with_defaults() -> None:
    ...
    config = load_config(PromptRegistryConfig, service_name="prompt-registry")

    assert config.service_name == "prompt-registry"
...
def test_reads_its_own_env_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CS_PROMPT_REGISTRY_SERVICE_NAME", "prompt-registry")
    monkeypatch.setenv("CS_PROMPT_REGISTRY_HTTP_PORT", "9100")

    config = load_config(PromptRegistryConfig)
...
def test_rejects_colliding_ports() -> None:
    ...
    with pytest.raises(ConfigurationError):
        load_config(
            PromptRegistryConfig, service_name="prompt-registry", http_port=8080, health_port=8080
        )
