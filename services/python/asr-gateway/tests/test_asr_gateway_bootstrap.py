"""Bootstrap tests for the asr-gateway service.
...
def test_config_loads_with_defaults() -> None:
    ...
    config = load_config(AsrGatewayConfig, service_name="asr-gateway")

    assert config.service_name == "asr-gateway"
...
def test_reads_its_own_env_prefix(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CS_ASR_GATEWAY_SERVICE_NAME", "asr-gateway")
    monkeypatch.setenv("CS_ASR_GATEWAY_HTTP_PORT", "9100")

    config = load_config(AsrGatewayConfig)
...
def test_rejects_colliding_ports() -> None:
    ...
    with pytest.raises(ConfigurationError):
        load_config(AsrGatewayConfig, service_name="asr-gateway", http_port=8080, health_port=8080)
